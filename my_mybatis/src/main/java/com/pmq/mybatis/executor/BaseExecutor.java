...
public abstract class BaseExecutor implements Executor {
	...
	public <T> List<T> query(MapperedStatement mapperedStatement, Configuration configuration, Object param) {
		// 获取带有值的sql语句
		String sql = mapperedStatement.getSqlSource().getBoundSql(param).getSql();
		// 从一级缓存去根据sql语句获取查询结果
		...
		// 如果没有结果，则调用相应的处理器去处理
		result = queryFromDataBase(mapperedStatement, configuration, param);
		oneLevelCache.put(sql, result);
		return (List<T>) result;
	}

	public abstract List<Object> queryFromDataBase(MapperedStatement mappedStatement, Configuration configuration,
			Object param);
